...
- Double click `backend/etl/run_all_etl.bat`
...
```bash
python backend/etl/run_all_etl.py --db "D:\Fastapi\src\data\mos_attendance.db" --dept 2531 2537 --truncate --replace
```
...
```bash
python <repo>\backend\etl\run_all_etl.py --truncate --replace --no-roster-filter
```
