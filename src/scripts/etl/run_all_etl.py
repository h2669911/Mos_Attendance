...
Usage:
  python src/scripts/etl/run_all_etl.py
  python src/scripts/etl/run_all_etl.py --db src/data/mos_attendance.db --truncate --replace
  python src/scripts/etl/run_all_etl.py --dept 2531 2537 --include-q --mark-inactive
"""
...
def main():
    ...
    ap.add_argument("--db", default="src/data/mos_attendance.db", help="SQLite DB path override")
    ...
    scripts = {
        "schema": repo_root / "src/scripts/etl/ensure_schema.py",
        "people": repo_root / "src/scripts/etl/etl_notes_people.py",
        "rawdata": repo_root / "src/scripts/etl/etl_notes_rawdata.py",
        "overtime": repo_root / "src/scripts/etl/etl_notes_overtime.py",
        "holiday": repo_root / "src/scripts/etl/etl_notes_holiday.py",
        "agg": repo_root / "src/scripts/etl/aggregate_metrics.py",
    }
    ...
    db_arg = Path(args.db)
    db_abs = db_arg if db_arg.is_absolute() else (repo_root / db_arg).resolve()

    common_db = ["--db", str(db_abs)]
    ...
    write_validation_report(db_abs, report_path)
